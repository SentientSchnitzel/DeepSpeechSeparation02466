...
parser.add_argument(
    "--test_dir",
    default="data/wav8k/max/test",
    type=str,
    required=False,
    help="Test directory including the csv files"
) # changed required to False, added defaults (change dir for remote work)
parser.add_argument(
    "--task",
    default="sep_clean",
    type=str,
...
) # changed required to False, added defaults
parser.add_argument(
    "--out_dir",
    default="OUTPUT_EVAL",
    type=str,
...
) # change to 1 for gpu - both in signal-level eval and WER
parser.add_argument(
    "--exp_dir",
    type=str,
    required=True,
...
def main(conf):
    ...
    wer_tracker = (
        MockWERTracker() if not conf["compute_wer"] else WERTracker(ASR_MODEL_PATH, anno_df, use_gpu=conf["use_gpu"])
    )
